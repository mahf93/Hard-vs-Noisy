...
class TinyImageNetAugNoisyDataset(Dataset):
    def __init__(self, root_dir, transform, normalize, mode='train', preload=True, load_transform=None,
               max_samples=None, transform_aug = None, max_noise_rate=0.4):
        tinp = TinyImageNetPaths(root_dir)
        self.mode = mode
        ...
        if self.preload:
            ...
            # placeholder id until replaced with actual id
            self.img_id = np.array(['aaaaaaaaaaaaaa' for _ in range(self.samples_num)]) 
            
            index = -1
            for idx in tqdm(range(len(self.samples)), desc=load_desc):
                index += 1
                s = self.samples[idx]
                img = Image.open(s[0])
                img_ = transform(img)
                img_ = _add_channels(img_)
                img = normalize(img_)
                self.img_data[index] = img
                if mode != 'test':
                    self.label_data[index] = s[self.label_idx]
                    self.img_id[index] = s[0][-15:-5]
                    
                    num_augs = num_augs_per_class(self.label_data[index])
                    
                    for aug in range(num_augs):
                        augmented_img = transform_aug[aug](img_)
                        augmented_img = normalize(augmented_img)
                        
                        index += 1
                        self.img_data[index] = augmented_img
                        self.label_data[index] = s[self.label_idx]
                        imgid = s[0][-15:-5]+'_'+str(aug)
                        self.img_id[index] = imgid
